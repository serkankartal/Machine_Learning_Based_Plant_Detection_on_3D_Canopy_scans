...
class SegmentMungbean():

    def __init__(self,tr_ref_point):
        globals = Globals
        self.tray_ref_point = tr_ref_point #globals.trayRefPoint
        self.path = globals.projectOutputPath
    ...
    def SegmentGround(self,fileName,cloud_name,type="plant"):
        ...
        while y_start + 200 <y_stop:
            ...
            if temp > y_limit:
                ...
                # if type=="plant":
                #     crop_points=soilSegm.SegmentSoil(cloud_bottom_part_full)
                # else:
                #     cloud = pcl.PointCloud_PointXYZI()
                #     cloud.from_array(np.concatenate((cloud_bottom_part_full.to_array(),cloud_top_part.to_array()), axis=0))
                #     crop_points=soilSegm.SegmentSoilLeaf(cloud,10)
                ...
                # cp.from_list(cloud_bottom_part_full)

                # directory =  self.path+ '/segmentation/segmented/' + cloud_name
                # if not os.path.exists(directory):
                #     os.makedirs(directory)
                # directory += "/"+str(part_count)+".ply"
                # pcl.save(cloud_bottom_part_full,directory)
